...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: verification, error: verificationError } = await supabase
      ...
      .eq("purpose", "account_reactivation")
    ...
    if (updateUserError) {
      return jsonResponse(500, { error: "No se pudo activar la cuenta" });
    }
    ...
    if (updateCodeError) {
      return jsonResponse(500, { error: "La cuenta se activo, pero no se pudo cerrar el codigo" });
    }
